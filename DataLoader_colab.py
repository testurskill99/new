...
class DataLoader(object):
    ...
    def _get_test_ds(self):
        test_dss = [p.Path(sub_dir) for sub_dir in p.Path(self.test_ds_dir).glob(
                                        f'{self.file_format}/{self.image_name}.{self.file_format}')]
        ...
        for test_ds_dir in test_dss:
            ...
            DS[name] = self._get_test_one_ds(test_ds_dir, self.img_types,
                                             self.MAX_SAMPLES, self.file_format)
        return DS

    def _get_test_one_ds(self, dir_, img_types, MAX_SAMPLES, file_format):

        # get the list of images
        file_list_ds, ds_size = _get_image_list(dir_,
                                                img_types,
                                                MAX_SAMPLES, self.image_name, file_format)
        ...
        ds_test = ds_test.map(
            lambda x: tf.py_function(
                func=_get_image_fn,
                inp=[x, self.WH, self.num_channels, self.conv_func, self.image_name],
    ...
    def _test_noise_adder_fn(self, DS, noise,
                             ...
                             compression,
                             num_debug=25):
        """ Adding noise and gibbs with different stddevs and frequncies """
...
def _get_image_fn(file_path: tf.Tensor,
    ...
    if conv_fun == 'png_to_png':
        img = cnv.png_to_png(image_name)
    if conv_fun == 'dcm_to_png':
        img = cnv.dcm_to_png(image_name)
    if conv_fun == 'dcm_to_jpeg':
        img = cnv.dcm_to_jpeg(image_name)
...
def _get_image_list(data_dir, img_types, MAX_SAMPLES, image_name, file_format, shuffle=0):
    """
    ...
    """
    ...
    # get filenames
    files_pathlib = list(
        [list(data_dir.glob(f'**/{image_name}.{file_format}'))]
    )
